...
Vagrant.configure(2) do |config|
    ...
    config.vm.box = "centos7_official"
    ...
    # CentOS7標準のrsyncディレクトリを無効化
    config.vm.synced_folder ".", "/home/vagrant/sync", disabled: true
    # 共有ディレクトリ
    config.vm.synced_folder "./share", "/home/vagrant/share", :create => true, :owner => "vagrant", :group => "vagrant", :mount_options => ["dmode=777", "fmode=777"]
    ...
    ###############################################################
    # 各種言語をインストール
    # * PHPはLAMPインストールでインストールするようにしているため、個別インストールは特に入れていない
    ###############################################################
